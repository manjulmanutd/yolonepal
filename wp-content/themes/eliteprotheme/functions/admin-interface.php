...
?>
<?php
function elite_optionsframework_add_admin() {
    global $query_string;
    
    $themename =  elite_get_option('of_themename');      
    $shortname =  elite_get_option('of_shortname'); 
   
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework' ) {
		if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
			$options =  elite_get_option('of_template'); 
			elite_reset_options($options,'optionsframework');
...
$of_page = add_theme_page($themename,'Theme Options','edit_theme_options', 'optionsframework', 'elite_optionsframework_options_page', 'div');
	
	// Add framework functionaily to the head individually
	add_action("admin_print_scripts-$of_page", 'elite_load_only');
} 
add_action('admin_menu', 'elite_optionsframework_add_admin');
/*-----------------------------------------------------------------------------------*/
/* Options Framework Reset Function - of_reset_options */
/*-----------------------------------------------------------------------------------*/
function elite_reset_options($options,$page = ''){
	global $wpdb;
	$count = 0;
	...
	if($page == 'optionsframework'){
		elite_delete_option( 'of_options' );
	}
}
/*-----------------------------------------------------------------------------------*/
/* Build the Options Page - optionsframework_options_page */
/*-----------------------------------------------------------------------------------*/
function elite_optionsframework_options_page(){
    $options =  elite_get_option('of_template');      
    $themename =  elite_get_option('of_themename');
?>
<div class="wrap" id="of_container">
  ...
  <form action="" enctype="multipart/form-data" id="ofform">
    <div id="header">
      ...
      <a href="http://www.elite.com" target="_new">
    ...
    <?php 
        ...
        $return = elite_optionsframework_machine($options);
        ?>
    <div id="main">
      <div id="of-nav">
        <ul>
...
/*-----------------------------------------------------------------------------------*/
/* Ajax Save Action - elite_ajax_callback */
/*-----------------------------------------------------------------------------------*/
add_action('wp_ajax_of_ajax_post_action', 'elite_ajax_callback');
function elite_ajax_callback() {
	...
	if($save_type == 'upload'){
		...
		$uploaded_file = wp_handle_upload($filename,$override);
				...
				elite_update_option( $clickedID , $uploaded_file['url'] );
		...
		foreach($options as $option_array){
			...
			if(isset($option_array['id'])) { // Non - Headings...
					...
					if ( is_array($type)){
						foreach($type as $array){
							if($array['type'] == 'text'){
								...
								elite_update_option( $id, stripslashes($new_value));
							}
						}                 
					}
					elseif($new_value == '' && $type == 'checkbox'){ // Checkbox Save
						
						elite_update_option($id,'false');
					}
					elseif ($new_value == 'true' && $type == 'checkbox'){ // Checkbox Save
						
						elite_update_option($id,'true');
					}
					elseif($type == 'multicheck'){ // Multi Check Save
						...
						foreach ($option_options as $options_id => $options_value){
							...
							if(!isset($output[$multicheck_id])){
							  elite_update_option($multicheck_id,'false');
							}
							else{
							   elite_update_option($multicheck_id,'true'); 
							}
						}
					} 
					elseif($type == 'typography'){
						...
						elite_update_option($id,$typography_array);
					...
					elseif($type == 'border'){
						...
						elite_update_option($id,$border_array);
...
/*-----------------------------------------------------------------------------------*/
function elite_optionsframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		break;
			...
			$select_value = elite_get_option($value['id']);
		...
		case "upload":
			$value['std'] = '';
			if (isset($value['std'])){
			$output .= elite_optionsframework_uploader_function($value['id'],$value['std'],null);
			}
		break;
		case "upload_min":
			
			$output .= elite_optionsframework_uploader_function($value['id'],$value['std'],'min');
			
		break;
		case "color":
			$val = $value['std'];
			$stored  = elite_get_option( $value['id'] );
		...
		case "typography":
			...
			$default = $value['std'];
			$typography_stored = elite_get_option($value['id']);
		...
		case "border":
			...
			$default = $value['std'];
			$border_stored = elite_get_option( $value['id'] );
...
/*-----------------------------------------------------------------------------------*/
/* OptionsFramework Uploader - elite_optionsframework_uploader_function */
/*-----------------------------------------------------------------------------------*/
function elite_optionsframework_uploader_function($id,$std,$mod){
	...
	$uploader = '';
    $upload = elite_get_option($id);
	
	if($mod != 'min') { 
			$val = $std;
            if ( elite_get_option( $id ) != "") { $val = elite_get_option($id); }
